...
void disableCallback(newtComponent co, void *data) {
    struct callbackInfo *cbi = reinterpret_cast<struct callbackInfo *>(data);
...
void helpCallback(newtComponent co, void *tag) {
    newtWinMessage("Help", "Ok", reinterpret_cast<char *>(tag));
}
...
int main(void) {
    ...
    f = newtForm(NULL, const_cast<char *>("This is some help text"), 0);
    ...
    lb = newtListbox(45, 1, 6, NEWT_FLAG_MULTIPLE | NEWT_FLAG_BORDER | NEWT_FLAG_SCROLL | NEWT_FLAG_SHOWCURSOR);
    ...
    do {
        ...
        if(es.reason == newtExitStruct::NEWT_EXIT_COMPONENT && es.u.co == b2) {
            newtScaleSet(scale, atoi(scaleVal));
            newtRefresh();
        } else if(es.reason == newtExitStruct::NEWT_EXIT_TIMER) {
            spinState++;
            if(!*spinState)
                spinState = spinner;
    ...
    } while(es.reason != newtExitStruct::NEWT_EXIT_COMPONENT || es.u.co == b2);
